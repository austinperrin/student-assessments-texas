...
1. Obtain the official TEA TFAR PDF for that year.
...
```powershell
Get-Content 'tfar/2025-tfar-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem tfar -Filter '*-tfar-fixed-width-mapping.json' |
  ForEach-Object {
