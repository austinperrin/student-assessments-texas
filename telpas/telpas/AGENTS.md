...
1. Obtain the official TEA TELPAS PDF for that year.
...
```powershell
Get-Content 'telpas/telpas/2026-telpas-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem telpas/telpas -Filter '*-telpas-fixed-width-mapping.json' |
  ForEach-Object {
