...
1. Obtain the official TEA TELPAS Alternate PDF for that year.
...
```powershell
Get-Content 'telpas/telpas_alt/2026-telpas-alt-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem telpas/telpas_alt -Filter '*-telpas-alt-fixed-width-mapping.json' |
  ForEach-Object {
