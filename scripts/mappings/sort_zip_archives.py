...
class OutputArchiveManager:
    def __init__(self, outputs_root: Path):
        ...
        self.states: dict[str, ArchiveOutputState] = {}
    ...
    def write_from_stream(
        ...
        state = self.states.get(archive_name)
        if state is None:
            destination = self.outputs_root / archive_name
            ...
            self.states[archive_name] = state
    ...
    def build_created_archives(self) -> list[dict[str, object]]:
        archives: list[dict[str, object]] = []
        for archive_name in sorted(self.states):
            state = self.states[archive_name]
            archives.append(
                {
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Extract ZIP archives from a .tmp input directory, classify files using "
            "mapping filename_patterns, and repack one archive per mapping file."
        )
    )
    parser.add_argument(
        "input_dir",
        ...
        help=(
            "Path to a .tmp subdirectory containing one or more .zip files. "
...
def process_archives_streaming(
    ...
    writer = OutputArchiveManager(outputs_root)
            ...
            with zipfile.ZipFile(zip_source) as zf:
                for member in zf.infolist():
                    ...
                    if winner is None:
                        ...
                        with zf.open(member) as src:
                            writer.write_from_stream("unsorted.zip", extracted_file, src)
                        continue
                    ...
                    with zf.open(member) as src:
                        writer.write_from_stream(f"{winner.output_stem}.zip", extracted_file, src)

        return (
            writer.build_created_archives(),
            nested_archives,
            metadata_files,
            unmatched_files,
            ambiguous_matches,
...
def create_zip_archive(destination: Path, files: list[ExtractedFile], preserve_source_dir: bool = False) -> list[str]:
    destination.parent.mkdir(parents=True, exist_ok=True)
...
def move_processed_archives(source_archives: list[Path], processed_root: Path, run_dir_name: str) -> list[str]:
    ...
    for archive in source_archives:
        destination = processed_run_dir / archive.name
...
def build_log_lines(summary: dict[str, object]) -> list[str]:
    lines = [
        ...
        f"Created archive count: {summary['created_archive_count']}",
    ...
    lines.append("")
    lines.append("Created archives:")
    for item in summary["created_archives"]:
        lines.append(f"- {item['archive']} ({item['file_count']} files)")
...
def process_input_dir(
    ...
    source_archives = sorted(path for path in input_dir.iterdir() if path.is_file() and is_zip_path(path))
    ...
    if keep_extracted:
        ...
        for extracted in extracted_files:
            ...
            if is_known_metadata_file(extracted_name):
                key = "metadata"
                archive_names[key] = "metadata.zip"
            ...
            if winner is None:
                key = "unsorted"
                archive_names[key] = "unsorted.zip"
                unmatched_files.append(extracted.archive_path)
            else:
                key = str(winner.mapping_path)
                archive_names[key] = f"{winner.output_stem}.zip"
                if len(matching_buckets) > 1:
                    ambiguous_matches.append(
                        {
        ...
        created_archives: list[dict[str, object]] = []
        for key, files in sorted(grouped_files.items(), key=lambda item: archive_names[item[0]].lower()):
            archive_name = archive_names[key]
            destination = outputs_root / archive_name
            members_written = create_zip_archive(
                destination,
                files,
                preserve_source_dir=(archive_name == "metadata.zip"),
            )
        ...
        created_archives, nested_archives, metadata_files, unmatched_files, ambiguous_matches = (
            process_archives_streaming(source_archives, buckets, outputs_root)
        )

    moved_archives = move_processed_archives(source_archives, processed_root, run_dir.name)
    end_dt = datetime.now()
    ...
    summary = {
        ...
        "processed_archive_destinations": moved_archives,
        "nested_archives": nested_archives,
        "created_archives": created_archives,
        "created_archive_count": len(created_archives),
        ...
        "matched_file_count": sum(
            item["file_count"]
            for item in created_archives
            if item["archive"] not in {"unsorted.zip", "metadata.zip"}
        ),
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"ZIP sorting failed: {exc}", file=sys.stderr)
        return 1

    print(f"ZIP sorting complete. Run output: {run_dir}")
    return 0
