...
1. Obtain the official TEA CRS Custom PDF for that year.
...
```powershell
Get-Content 'crs/2026-crs-custom-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem crs -Filter '*-crs-custom-fixed-width-mapping.json' |
  ForEach-Object {
