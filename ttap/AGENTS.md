...
1. Obtain the official TEA TTAP PDF for that year.
...
```powershell
Get-Content 'ttap/2025-ttap-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem ttap -Filter '*-ttap-fixed-width-mapping.json' |
  ForEach-Object {
