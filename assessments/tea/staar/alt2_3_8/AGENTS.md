# STAAR Alternate 2 3-8 AI Agent Guide

This file guides AI agents working inside `assessments/tea/staar/alt2_3_8/`.

## Purpose

This folder contains year-specific JSON mappings for STAAR Alternate 2 grades 3-8 reporting student data files published by the Texas Education Agency.
...
- prefer stable subject namespaces when the source labels vary across years
- do not collapse fields with meaningfully different TEA definitions unless the schema choice is explicit

## Family-Specific Notes

- the first confirmed official source year for this family in this repo is `2016`
- do not create `2015` or earlier mappings without a year-specific PDF
- when subject names vary across years, prefer stable namespaces such as `reading_language_arts`
...
1. Obtain the official TEA STAAR Alternate 2 grades 3-8 online source for that year.
...
```powershell
Get-Content 'assessments/tea/staar/alt2_3_8/2026-staar-alt2-3-8-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem assessments/tea/staar/alt2_3_8 -Filter '*-staar-alt2-3-8-fixed-width-mapping.json' |
  ForEach-Object {
