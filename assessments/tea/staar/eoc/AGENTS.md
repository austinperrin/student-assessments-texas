# STAAR EOC AI Agent Guide

This file guides AI agents working inside `assessments/tea/staar/eoc/`.

## Purpose

This folder contains year-specific JSON mappings for STAAR End-of-Course reporting student data files published by the Texas Education Agency.
...
- preserve distinct EOC subject namespaces and year-specific terminology when the PDF meaning differs

## Family-Specific Notes

- `administration_periods` is required for this family and should reflect the current PDF exactly
- trim wrapped artifacts such as partial `_raw`, `_at_the`, and similar cut-off fragments back to the actual field title
- normalize `T/T/T Tester` labels to `ttt_tester_*`
...
1. Obtain the official TEA EOC online source for that year.
...
```powershell
Get-Content 'assessments/tea/staar/eoc/2026-staar-eoc-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem assessments/tea/staar/eoc -Filter '*-staar-eoc-fixed-width-mapping.json' |
  ForEach-Object {
