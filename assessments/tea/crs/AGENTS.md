# CRS Custom AI Agent Guide

This file guides AI agents working inside `assessments/tea/crs/`.

## Purpose

This folder contains year-specific JSON mappings for the CRS Custom family.
...
- normalize portal access code fields to `family_portal_unique_access_code` when present in the source layout

## Family-Specific Notes

- use this folder only for official TEA PDFs labeled as `CRS Custom Data File Layout` or `CRS Data File Format`
- do not use this folder for interim-only student-results layouts that happen to contain `CRS` in the filename
- preserve real year differences such as `opportunity_key` versus `result_id`
...
1. Obtain the official TEA CRS Custom online source for that year.
...
```powershell
Get-Content 'assessments/tea/crs/2026-crs-custom-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem assessments/tea/crs -Filter '*-crs-custom-fixed-width-mapping.json' |
  ForEach-Object {
