...
This file guides AI agents working inside `assessments/tea/telpas/telpas/`.

## Purpose

This folder contains year-specific JSON mappings for TELPAS reporting student data files.
...
- `2024-telpas-fixed-width-mapping.json`
...
- preserve TELPAS-specific meaning when TEA changes a field concept rather than just the label

## Family-Specific Notes

- use the ending year for school-year-labeled source PDFs such as `2023-2024`
- preserve TELPAS rating and proficiency field meaning exactly as described in the PDF
...
1. Obtain the official TEA TELPAS online source for that year.
...
```powershell
Get-Content 'assessments/tea/telpas/telpas/2026-telpas-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem assessments/tea/telpas/telpas -Filter '*-telpas-fixed-width-mapping.json' |
  ForEach-Object {
