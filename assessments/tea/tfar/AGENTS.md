...
This file guides AI agents working inside `assessments/tea/tfar/`.

## Purpose

This folder contains year-specific JSON mappings for TFAR data files.
...
- `2024-tfar-fixed-width-mapping.json`
...
- use the ending year for school-year-labeled TFAR source files
- preserve real structural differences between `2024` and `2025` rather than forcing them into one shape
...
1. Obtain the official TEA TFAR online source for that year.
...
```powershell
Get-Content 'assessments/tea/tfar/2025-tfar-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem assessments/tea/tfar -Filter '*-tfar-fixed-width-mapping.json' |
  ForEach-Object {
