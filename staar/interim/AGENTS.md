...
1. Obtain the official TEA STAAR Interim PDF for that year.
...
```powershell
Get-Content 'staar/interim/2026-staar-interim-fixed-width-mapping.json' | ConvertFrom-Json | Out-Null
```

For all files in this family:

```powershell
Get-ChildItem staar/interim -Filter '*-staar-interim-fixed-width-mapping.json' |
  ForEach-Object {
